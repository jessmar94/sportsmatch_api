...
class GamesTest(unittest.TestCase):
  ...
  def test_return_all_opponent_games(self):
    updated_game = {
        "status": "confirmed"
    }
    res = self.client().post('api/v1/players/login', headers={'Content-Type': 'application/json'}, data=json.dumps(self.player_1))
    ...
    json_data = json.loads(res.data)
    self.assertEqual(json_data[0].get('opponent_id'), 1)
    self.assertEqual(json_data[0].get('organiser_id'), 2)
    self.assertEqual(res.status_code, 200)

  def test_return_all_organiser_games(self):
    updated_game = {
        "status": "confirmed"
    }
    res = self.client().post('api/v1/players/login', headers={'Content-Type': 'application/json'}, data=json.dumps(self.player_1))
    ...
    json_data = json.loads(res.data)
    self.assertEqual(json_data[0].get('opponent_id'), 2)
