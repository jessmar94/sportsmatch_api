...
class GameModel(db.Model):
    ...
    Game Model
    """
    # name our table Games
    __tablename__ = 'games'

    id = db.Column(db.Integer, primary_key=True)
    organiser_id = db.Column(db.Integer, db.ForeignKey('players.id'),
                             nullable=False)
    ...
    organiser = db.relationship("PlayerModel",
                                primaryjoin="GameModel.organiser_id == PlayerModel.id",
                                backref="organiser")
    opponent = db.relationship("PlayerModel",
                               primaryjoin="GameModel.opponent_id == PlayerModel.id",
                               backref="opponent")
    result = db.relationship("ResultModel", uselist=False,
                             back_populates="game")
    message = db.relationship("MessageModel", back_populates="game")

    # class constructor used to set the class attributes
    ...
    @staticmethod  # static methods are class methods
    def get_all_games():
        return GameModel.query.all()

    @staticmethod
    def get_all_users_games(id):
        return GameModel.query.filter(or_(GameModel.organiser_id == id,
                                          GameModel.opponent_id == id))\
    ...
    def get_game_by_org_id(user_id):
        return GameModel.query.filter_by(organiser_id=user_id)\
         .filter(GameModel.status == "confirmed",
                 GameModel.game_date <= datetime.datetime.utcnow())
