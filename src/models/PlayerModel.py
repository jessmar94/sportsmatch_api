...
class PlayerModel(db.Model):  # PlayerModel class inherits from db.Model
    ...
    def get_player_profile_image(id):
        return PlayerModel.query.with_entities(PlayerModel.profile_image)\
    ...
    def get_player_name(id):
        ...
        player = PlayerModel.query.with_entities(PlayerModel.first_name)\
                            .filter_by(id=id).first()
    ...
    def get_filtered_players(id, ability, distance):
        ...
        return PlayerModel.\
            get_players_within_distance(players, serialized_user, distance)

    @staticmethod
    def get_player_location(postcode):
        req_data = requests\
         .get(f'https://api.postcodes.io/postcodes/{postcode}').json()
    ...
    def get_players_within_distance(players, user, distance):
        ...
        for player in players:
            distances_between_players = int(round(
             PlayerModel.get_distance_between_postcodes(
              player.postcode, user_postcode)))
    ...
    def __repr__(self):  # returns printable representation of PM object (id)
...
class BytesField(fields.Field):
    ...
    def _serialize(self, value, attr, obj, **kwargs):
        ...
        base64_string = binary_string.to_bytes((
         binary_string.bit_length() + 7) // 8, 'big').decode()
